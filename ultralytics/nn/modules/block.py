...
class VimBlock(nn.Module):
    """
    Visual Mamba Block for modeling long-range dependencies with linear complexity.
    
    This module approximates the Mamba SSM (Selective State-Space Model) effect using
    large kernel convolutions to provide global context modeling.
    
    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels. If None, equals to c1.
        kernel_size (int): Size of the depthwise convolution kernel. Default: 11.
    """
    
    def __init__(self, c1, c2=None, kernel_size=11):
        super().__init__()
        if c2 is None:
            c2 = c1
        ...
        # Large kernel depthwise conv to approximate global attention
        # 注意：不使用分组卷积以避免通道数缩放问题
        self.dw_conv = nn.Conv2d(
            c1, c1, 
            kernel_size=kernel_size, 
            padding=kernel_size//2, 
            groups=1,  # 使用标准卷积而非深度卷积
            bias=False
        )
        ...
        # Channel attention (simplified SE module)
    ...
    def forward(self, x):
        """Forward pass through Vim block.""" 
        # 动态适应输入通道数
        input_channels = x.shape[1]
        
        # 如果输入通道数与预期不符，需要调整
        if input_channels != self.c1:
            # 使用自适应卷积处理不匹配的通道数
            if not hasattr(self, 'adaptive_conv'):
                self.adaptive_conv = nn.Conv2d(
                    input_channels, self.c2, 
                    kernel_size=1, bias=False
                ).to(x.device)
                self.adaptive_bn = nn.BatchNorm2d(self.c2).to(x.device)
            
            out = self.adaptive_bn(self.adaptive_conv(x))
            return self.act(out)
        ...
        # Residual connection (only if input/output channels match)
        if self.c1 == self.c2:
            out = out + x
...
class CA_FPN_Block(nn.Module):
    """
    Context-Aware Feature Pyramid Network Block.
    
    This module implements adaptive feature fusion between high-level and low-level features
    using both spatial and channel attention mechanisms.
    
    Args:
        in_low (int): Number of channels in low-level features.
        in_high (int): Number of channels in high-level features.
        out (int): Number of output channels.
    """
    
    def __init__(self, in_low, in_high, out):
        super().__init__()
        self.in_channels = in_low + in_high
        
        # Spatial attention: large kernel depth-wise conv for context modeling
        self.spatial_att = nn.Conv2d(
            self.in_channels, self.in_channels, 
            kernel_size=5, padding=2, 
            groups=self.in_channels, bias=False
        )
        
        # Channel attention: SE module for global channel relationships
        self.channel_att = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(self.in_channels, self.in_channels // 4, kernel_size=1),
            nn.SiLU(),
            nn.Conv2d(self.in_channels // 4, self.in_channels, kernel_size=1),
            nn.Sigmoid()
        )
        
        # Output fusion convolution
        self.conv_out = nn.Conv2d(self.in_channels, out, kernel_size=1, bias=False)
        self.bn_out = nn.BatchNorm2d(out)
        self.act_out = nn.SiLU()
    
    def forward(self, high_feat, low_feat):
        """
        Forward pass through CA-FPN block.
        
        Args:
            high_feat: High-level features (already upsampled to match low_feat size)
            low_feat: Low-level features
        
        Returns:
            Fused features with adaptive attention weighting
        """
        ...
        w = self.channel_att(x_spatial)
        x_att = x_spatial * w
        
        # Output projection
        out = self.conv_out(x_att)
        out = self.bn_out(out)
...
class CompactInvertedBlock(nn.Module):
    """
    Compact Inverted Block (CIB) similar to MobileNet inverted residuals.
    
    This block is designed to preserve high-frequency details while being computationally efficient.
    
    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels.  
        expansion (float): Expansion ratio for the hidden dimension. Default: 2.0.
    """
    
    def __init__(self, c1, c2=None, expansion=2.0):
        super().__init__()
        if c2 is None:
            c2 = c1  # 如果没有指定输出通道，则与输入通道相同
        
        self.c1, self.c2 = c1, c2
        mid = int(c1 * expansion)
        
        # Expansion convolution
        self.expand = Conv(c1, mid, k=1)
        
        # Depthwise convolution for spatial information
        self.depthwise = Conv(mid, mid, k=3, g=mid)
        ...
        self.act = nn.SiLU()
        self.use_res = (c1 == c2)  # 只有当输入输出通道相同时才使用残差连接
    
    def forward(self, x):
        """Forward pass through compact inverted block."""
        out = self.expand(x)
        out = self.depthwise(out)
        out = self.linear(out)
        
        if self.use_res:
            out = out + x  # Residual connection preserves details
...
class SimAM(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算注意力权重
        attn = (x - mu) ** 2 / (4 * var + self.eps) + 0.5
        
        return x * attn


class FusionLockTSS(nn.Module):
    """
    MGDFIS的Fusion-Lock TSS模块
    通过自注意力机制锁定重要的纹理和结构特征
    """
    ...
    def forward(self, x):
        ...
        # Tanh激活后与原输入相乘
        return self.tanh(attn_out) * x


class GlobalDetail(nn.Module):
    """
    MGDFIS的Global-Detail模块
    结合全局上下文和局部细节信息
    """
...
class DynamicPixelAttn(nn.Module):
    """
    MGDFIS的Dynamic Pixel Attention模块
    为每个像素动态生成注意力权重
    """
...
class MFBlock(nn.Module):
    """
    MambaFusion Block - C2fSTBlock的实现
    融合CNN、Swin Transformer和MGDFIS三组件
    专为小目标检测优化的复合模块
    
    Args:
        ch_in (int): 输入通道数
        ch_hidden (int): 隐藏层通道数
    """
    
    def __init__(self, ch_in, ch_hidden):
        super().__init__()
        
        # 通道调整
        self.conv1 = Conv(ch_in, ch_hidden, 1)
        
        # 轻量化Swin Transformer（简化版本）
        self.swin_conv = nn.Sequential(
            nn.Conv2d(ch_hidden, ch_hidden, 7, 1, 3, groups=ch_hidden),  # 深度卷积模拟窗口注意力
            nn.BatchNorm2d(ch_hidden),
            nn.SiLU(),
            nn.Conv2d(ch_hidden, ch_hidden, 1),  # 点卷积进行通道混合
            nn.BatchNorm2d(ch_hidden)
        )
        
        # MGDFIS三子模块
        self.fusion_lock = FusionLockTSS(ch_hidden)
        self.global_detail = GlobalDetail(ch_hidden) 
        self.dynamic_pixel = DynamicPixelAttn(ch_hidden)
        
        # 输出通道调整
        self.conv2 = Conv(ch_hidden, ch_in, 1)
    ...
    def forward(self, x):
        ...
        # Swin-like特征处理
        y = self.swin_conv(y) + y  # 残差连接
        
        # MGDFIS处理流水线
        y = self.fusion_lock(y)      # 融合锁定
        y = self.global_detail(y)    # 全局细节
        y = self.dynamic_pixel(y)    # 动态像素注意力
        
        # 通道降维
        y = self.conv2(y)
        
        # 残差连接 + SimAM注意力
        return self.simam(identity + y)
