...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(layers_def):  # from, number, module, args
        ...
        if m in {
            ...
            RecurrentAttentionFusionBlock,  # 自定义RAFB: 参与c1/c2注入与width缩放  # 中文注释
            BiLevelRoutingAttentionFusionBlock,  # 自定义BRAF: 参与c1/c2注入与width缩放  # 中文注释
        }:
            c1, c2 = ch[f], args[0]
            ...
            # SpaceToDepth: 通道 * (r^2)
            r = int(args[0]) if args else 2
            c2 = ch[f] * (r ** 2)
        elif m is OmniKernelFusion:
            # OmniKernelFusion: 输出通道由 args[1] 指定
            # args: [in_ch, out_ch, ...]
            c2 = int(args[1])
