...
# ---------- 默认参数（保持与你原脚本一致） ----------
DEFAULT_DATA="/workspace/yolo/ultralytics/cfg/datasets/uavvaste.yaml"   # 数据集 YAML（默认）
DEFAULT_EPOCHS=500          # 训练轮数默认
DEFAULT_IMGSZ=1024           # 输入尺寸默认
DEFAULT_BATCH=16          # 批大小默认
...
python /workspace/yolo/train_yolov12_staged.py \
  ...
  --name yolov12_sod_fusion_v5_Uavvaste
